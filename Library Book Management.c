...
void chinhsua(Node *h) {
    ...
    if(k == 0) { 
        ...
        if(c == 1) { printf("Nhap ma so sach: "); scanf("%s", h->book.bid); }
        if(c == 2) { printf("Nhap ten sach: "); scanf("%s", h->book.ten); }
        if(c == 3) { printf("Nhap gia sach: "); scanf("%d", &h->book.pb); }
        if(c == 4) { printf("Nhap so luong con lai: "); scanf("%d", &h->book.qre); }
        if(c == 5) {
            // Sửa toàn bộ thông tin sách
            printf("Nhap ma so sach: "); scanf("%s", h->book.bid);
            printf("Nhap ten sach: "); scanf("%s", h->book.ten);
            printf("Nhap gia sach: "); scanf("%d", &h->book.pb);
            printf("Nhap so luong con lai: "); scanf("%d", &h->book.qre); 
        }
...
int main() {
    ...
    do {
        ...
        printf("Nhap 0 de thoat va ghi file data.txt\n");
        printf("Moi nhap: ");
        scanf("%d", &c);  // Nhập lựa chọn từ bàn phím
        if(c == 0) {
